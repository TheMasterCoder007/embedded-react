...
- [x] QuickJS compiles and links against the engine — `er-bridge-quickjs-smoke` (headless) and
  now the `embedded-react-desktop` SDL host (QuickJS-ng v0.15.0 via FetchContent)
...
- [x] `NativeUI.commit()` drives `er_commit()` and the **SDL window shows the result** — the
  `embedded-react-desktop` target boots QuickJS, runs a JS app, and paints it via the SDL
...
- [x] `imageName` key → `ERProps.image_name`; `<Image source={...}>` (a baked-name string, an
  imported `.png` resolving to its basename, or an RN-style `{ uri }`) resolves to `imageName` in
  `buildProps`. Pixels are registered at boot via the build-time bake (§1.5)
...
**Resolved via build-time bake — not a runtime JS surface.** Shipping image pixels / font glyphs
through JS would bloat the QuickJS heap and slow boot; instead the bundler bakes them to flash-resident
C. An app `import`s a `.png`/`.ttf` (the import resolves to the asset's basename — the image name or
font family), and `npm run build` discovers those imports and emits `dist/assets.generated.c` exposing
`er_register_assets()`, which the host calls once at boot.

- [x] Images → `er_image_load` — `bake-image.mjs` (PNG → premultiplied ARGB8888) + `<Image source>`
  resolves to the baked name (see §1.2). Flash-resident, zero runtime RAM.
- [x] Fonts → `er_font_register` — `bake-font.mjs` (TTF/OTF → `BitmapFont` via `opentype.js` + a pure-JS
  rasterizer), at the literal `fontSize`s the bundle uses (bpp 4 default; `assets.config.js` overrides
  sizes/bpp/glyphs). The engine's built-in Inter (`font_data.c`) is regenerated by the same baker
  (`npm run build:builtin-font`). New public `er_font_register(family, BitmapFont*)` registers a baked
  font by pointer (no font pool; works with `ERUI_FONT_POOL_BYTES = 0`).
- Deferred (not needed for the bake path): a *runtime* `NativeUI.loadImage`/`loadFont` for dynamically
  generated/OTA assets. `er_font_load` (blob → pool) remains for that future case.
...
- [x] `JSRuntime` / `JSContext` init + teardown on exit (`examples/linux/main.c`)
...
demo (`embedded-react-desktop`) runs the bundle by default and is interactive.

...
onto a microcontroller. The UI runs _on the device_: no browser, no phone, and no operating system
required.
...
Embedded React is the opposite. It takes React Native's approach: React is a _component and
reconciliation model_, not a DOM thing. React Native swapped the browser's host primitives (div,
...
difference is _when_ the dynamism is resolved.
...
| Trade-off   | RAM and per-frame dispatch cost                                                         | A [subset of the API](./guides/aot-subset.md)                                  |

Choosing between them is a build flag, not a rewrite. [The two flows](./concepts/two-flows.md) goes
deeper.

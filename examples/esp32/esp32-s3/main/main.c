...
static void run_app(void)
{
    ...
    /* Register the demo's baked assets (images + fonts) now that the backend (and its registries) is
     * up, before the app mounts and creates <Image>/<Text> nodes that look them up by name. Generated
     * by `npm run build` alongside the app bytecode. */
#if defined(ER_HAVE_BAKED_ASSETS)
    er_register_assets();
#endif

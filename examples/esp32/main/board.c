...
#define EXIO_TP_RST_BIT (1U << 1) /* touch reset (Phase 3) */
...
static esp_err_t board_io_init(void)
{
    ...
    ESP_RETURN_ON_ERROR(i2c_master_bus_add_device(bus, &set_cfg, &s_ch422g_set), TAG, "ch422g set");
    ESP_RETURN_ON_ERROR(i2c_master_bus_add_device(bus, &io_cfg, &s_ch422g_io), TAG, "ch422g io");

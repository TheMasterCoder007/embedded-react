...
int main(void)
{
    ...
    /* Anchor Date.now() to the real time before the build, so a mount effect already reads it. A board with
       no RTC would call this once it learns the time (SNTP, GPS). */
    er_app_set_wall_clock((int64_t)time(NULL) * 1000);

...
- Animated 2D scale transforms sample through a separable fast path that hoists the per-pixel inverse-map,
  `floorf`, and bounds checks out of the inner loop (constant per row/column under a pure axis-aligned
  scale), at full bilinear quality (output unchanged). Rotation/shear keep the general affine path.

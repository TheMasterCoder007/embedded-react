 ...
 *
 * @param[in] src    Source pixel buffer (row stride = ERUI_XFORM_W).
...
static uint32_t er_bilerp(const uint32_t* src, int src_w, int src_h, float flx, float fly)
{
    ...
    const uint32_t c00 = (x0 >= 0 && x0 < src_w && y0 >= 0 && y0 < src_h) ? src[y0 * ERUI_XFORM_W + x0] : 0u;
    const uint32_t c10 = (x1 >= 0 && x1 < src_w && y0 >= 0 && y0 < src_h) ? src[y0 * ERUI_XFORM_W + x1] : 0u;
    const uint32_t c01 = (x0 >= 0 && x0 < src_w && y1 >= 0 && y1 < src_h) ? src[y1 * ERUI_XFORM_W + x0] : 0u;
    const uint32_t c11 = (x1 >= 0 && x1 < src_w && y1 >= 0 && y1 < src_h) ? src[y1 * ERUI_XFORM_W + x1] : 0u;

    return er_blend4(c00, c10, c01, c11, wx, wy);
}

/**
 * @brief Intersects the destination AABB with the active scissor before the emit loops.
 *
 * Rows/columns outside the scissor would be rejected at the blit anyway, so skipping their
 * back-projection entirely turns a banded composite pass from O(dst area) into O(strip area).
 *
 * @return false when nothing survives the intersection.
 */
static bool xform_emit_bounds(int dst_x, int dst_y, int dst_w, int dst_h, int* ex0, int* ey0, int* ex1, int* ey1)
{
    *ex0 = dst_x;
    *ey0 = dst_y;
    *ex1 = dst_x + dst_w;
...
{
    int ex0, ey0, ex1, ey1;
    if (!xform_emit_bounds(dst_x, dst_y, dst_w, dst_h, &ex0, &ey0, &ex1, &ey1))
        return;

    /* Scale-only fast path: with no rotation/shear (ib == ic == 0) the inverse map is separable —
     * flx depends only on the screen column, fly only on the screen row — so per column segment
     * we precompute flx, floorf(flx) and wx per column, and fly/floorf(fly)/wy plus row pointers
     * per row. The inner loop is then just four reads + the shared bilinear blend, so the output
     * is bit-identical to the general path. Rotation/shear falls through below. */
    if (ib == 0.0f && ic == 0.0f)
    {
        int cx0[ERUI_XFORM_W];
        uint16_t cwx[ERUI_XFORM_W];
        for (int seg = ex0; seg < ex1; seg += ERUI_XFORM_W)
        {
            const int seg_w = (ex1 - seg) < ERUI_XFORM_W ? (ex1 - seg) : ERUI_XFORM_W;
    ...
    /* General path: for each screen-space pixel in the (scissor-bounded) destination box,
     * inverse-map to the source scratch and sample bilinearly. */
    for (int oy = ey0; oy < ey1; oy++)
    {
        const float sy = (float)oy;
...
#if ERUI_TRANSFORMS_FULL
/**
 * @brief Back-projects the source scratch through an inverse homography and blends it out.
 *
 * Shared by the end-of-capture blit and the banded replay blit; the emit region is bounded
 * to the active scissor (see xform_emit_bounds).
 */
static void xform_emit_3d(
...
bool er_transform_source_begin(int src_x, int src_y, int w, int h)
{
     ...
     * band clip would truncate the source at the strip seam. */
    er_push_clip_reset();

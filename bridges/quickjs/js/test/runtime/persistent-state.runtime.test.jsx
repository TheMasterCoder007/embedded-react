...
check(
  lastValue === 10,
  `initial uses the default when nothing is persisted (got ${lastValue})`,
);
...
check(
  store.get('counter') === '42',
  `value persisted as JSON (got ${store.get('counter')})`,
);
...
check(
  lastValue === 42,
  `restored persisted value after remount (got ${lastValue})`,
);
...
NativeUI.tick(0);
check(
  lastValue === 43 && store.get('counter') === '43',
  `functional update persists (got ${lastValue})`,
);
...
check(
  lastValue === 10,
  `degrades to useState default when no persist host (got ${lastValue})`,
);

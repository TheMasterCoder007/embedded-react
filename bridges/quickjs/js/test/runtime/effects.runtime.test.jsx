...
NativeUI.tick(0);
check(mountRuns === 1, `mount effect ran after first commit + pump (runs=${mountRuns})`);
check(depRuns === 1 && lastDep === 1, `dep effect ran with n=1 (runs=${depRuns}, n=${lastDep})`);
...
NativeUI.tick(0);
check(depRuns === 2 && lastDep === 2, `dep effect re-ran when n changed to 2 (runs=${depRuns})`);

...
function App({ n, bold }) {
  return (
    <View style={{ width: 300, height: 200 }}>
      ...
      >
        Hello{' '}
        <Text style={{ fontWeight: 'bold', color: 'red' }}>{bold}</Text>!
      </Text>
    </View>
  );
}
...
check(wShort >= 0 && wRich >= 0, 'both Text nodes measured a non-negative width');
...
check(wLong > wShort, `longer interpolation measures wider (${wShort} -> ${wLong})`);

// The nested-span text measures its full concatenated content ("Hello world!"), wider than "Hello".
check(wRich > 0, `nested-span Text measured its full content width (rich=${wRich})`);

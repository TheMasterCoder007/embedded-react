...
export function emitAssetsC({headerName, images = [], fonts = []}) {
  ...
  lines.push(
    `/* Images: premultiplied ARGB8888 (or RGB565 when baked 16-bit), referenced by pointer. Fonts: baked BitmapFont per size. */`,
  );
  lines.push(``);
  ...
  for (const img of images) {
    const loadFn =
      img.format === 'rgb565' ? 'er_image_load_rgb565' : 'er_image_load';
    lines.push(
      `    ${loadFn}(${JSON.stringify(img.name)}, ${cIdent(img.name)}_px, ${img.width}, ${img.height});`,
    );
  }
